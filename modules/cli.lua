...
function cli.show_help()
    local help_text = {
        ...
        "",
        utils.colors.yellow .. "Example: lua tamagotchi.lua create" .. utils.colors.reset
    }
...
function cli.list_tamagotchis()
    ...
    if #pets == 0 then
        print("You have no pets. Create one with: lua tamagotchi.lua create")
        return
    end
    ...
    print()
    print("* = Active pet")
    print("Select a pet with: lua tamagotchi.lua select <name>")
end

function cli.select_tamagotchi(name)
    if not name then
        print("Usage: lua tamagotchi.lua select <name>")
        return
    end
...
function cli.get_active_pet()
    ...
    if not active_name then
        print("You don't have an active pet. Select one with: lua tamagotchi.lua select <name>")
        return nil
    end
...
function cli.feed_tamagotchi()
    ...
    if success then
        ...
        print("Buy food with: lua tamagotchi.lua buy apple")
    end
end
...
function cli.buy_item(item_name)
    if not item_name then
        print("Usage: lua tamagotchi.lua buy <item>")
        print("\nAvailable items:")
...
function cli.use_item(item_name)
    if not item_name then
        print("Usage: lua tamagotchi.lua use <item>")
        return
    end
...
function cli.breed_tamagotchis(father, mother, child)
    if not father or not mother or not child then
        print("Usage: lua tamagotchi.lua breed <father> <mother> <child>")
        return
    end
...
function cli.show_family_tree(pet_name)
    if not pet_name then
        ...
        if pet then
            ...
            print("Usage: lua tamagotchi.lua family <name>")
            return
        end
    end
...
function cli.show_compatibility(pet1, pet2)
    if not pet1 or not pet2 then
        print("Usage: lua tamagotchi.lua compatibility <pet1> <pet2>")
        return
    end
...
function cli.execute_command(command, params)
    ...
    elseif command == "help" then
        ...
        print("Use 'lua tamagotchi.lua help' to see all commands.")
    end
end
